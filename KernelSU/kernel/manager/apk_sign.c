...
{
	if (strstr(path, "resukisu")) {
		pr_info("is_manager_apk: matching resukisu path: %s\n", path);
		return true;
	}
	if (strstr(path, "ksunext")) {
		pr_info("is_manager_apk: matching ksunext path: %s\n", path);
		return true;
	}

	char pkg[KSU_MAX_PACKAGE_NAME];
	if (get_pkg_from_apk_path(pkg, path) >= 0) {
		if (!strncmp(pkg, "me.weishu.kernelsu", 18) ||
		    !strncmp(pkg, "io.github.mksu", 14) ||
		    !strncmp(pkg, "rsuntk.x.ksu", 12)) {
			return true;
		}
