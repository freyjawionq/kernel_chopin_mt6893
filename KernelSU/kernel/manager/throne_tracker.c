...
static void crown_manager(const char *apk, struct list_head *uid_data)
{
	...
	bool is_resukisu = (strstr(apk, "resukisu") != NULL);
	bool is_ksunext = (strstr(apk, "ksunext") != NULL);

	if (get_pkg_from_apk_path(pkg, apk) < 0) {
		pr_err("Failed to get package name from apk path: %s\n", apk);
		if (is_resukisu) {
			strcpy(pkg, "com.resukisu.resukisu");
		} else if (is_ksunext) {
			strcpy(pkg, "com.resukisu.resukisu");
		} else {
			return;
	...
	pr_info("manager pkg: %s (is_resukisu: %d)\n", pkg, is_resukisu);
	...
	list_for_each_entry (np, list, list) {
		if (strncmp(np->package, pkg, KSU_MAX_PACKAGE_NAME) == 0 ||
		    (is_resukisu && strstr(np->package, "resukisu")) ||
		    (is_ksunext && strstr(np->package, "ksunext"))) {
