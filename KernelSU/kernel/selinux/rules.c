...
static bool ksu_is_recovery(void)
{
    struct path path;
    if (kern_path("/sbin/recovery", LOOKUP_FOLLOW, &path) == 0) {
        path_put(&path);
        return true;
    }
    if (kern_path("/twres", LOOKUP_FOLLOW, &path) == 0) {
        path_put(&path);
        return true;
    }
    if (kern_path("/OF_init", LOOKUP_FOLLOW, &path) == 0) {
        path_put(&path);
        return true;
    }
    // Check if /system/bin/recovery exists AND it's not a full Android boot
    if (kern_path("/system/bin/recovery", LOOKUP_FOLLOW, &path) == 0) {
        path_put(&path);
        if (kern_path("/system/framework/framework-res.apk", LOOKUP_FOLLOW, &path) != 0) {
            return true;
        } else {
            path_put(&path);
        }
...
static int apply_kernelsu_rules_fn(void *ptr)
{
    ...
    {
        const char *apps[] = { "untrusted_app", "untrusted_app_25", "untrusted_app_27", "untrusted_app_29", "untrusted_app_30", "untrusted_app_32" };
        int i;
        for (i = 0; i < sizeof(apps)/sizeof(apps[0]); i++) {
            ksu_allow(db, apps[i], "proc_perfmgr", "file", ALL);
            ksu_allow(db, apps[i], "proc_ppm", "dir", ALL);
            ksu_allow(db, apps[i], "proc_ppm", "file", ALL);
            ksu_allow(db, apps[i], "sysfs_zram", "dir", ALL);
            ksu_allow(db, apps[i], "sysfs_zram", "file", ALL);
            ksu_allow(db, apps[i], "proc_swaps", "file", ALL);
            if (db->policyvers >= POLICYDB_VERSION_XPERMS_IOCTL) {
                ksu_allowxperm(db, apps[i], "proc_perfmgr", "file", ALL);
                ksu_allowxperm(db, apps[i], "proc_ppm", "file", ALL);
                ksu_allowxperm(db, apps[i], "sysfs_zram", "file", ALL);
                ksu_allowxperm(db, apps[i], "proc_swaps", "file", ALL);
            }
        }
    }
...
void apply_kernelsu_rules()
{
	...
	if (ksu_is_recovery()) {
		pr_info("KernelSU in recovery mode, skipping SELinux rules application\n");
		return;
	}
...
int handle_sepolicy(void __user *user_data, u64 data_len)
{
	...
	if (ksu_is_recovery()) {
		pr_info("KernelSU in recovery mode, skipping handle_sepolicy\n");
		return 0;
	}
