...
void charger_manager_set_input_suspend_pissarro(bool input_suspend)
{
	if (pinfo == NULL)
		return;
...
int charger_manager_get_charge_status(void)
{
	...
	if (pinfo->psy_type) {
		if (pinfo->typec_burn || pinfo->input_suspend || pinfo->bms_i2c_error_count >= 10 ||
			!is_between(pinfo->jeita_fcc_cfg[0].low_threshold, pinfo->jeita_fcc_cfg[STEP_JEITA_TUPLE_COUNT - 1].high_threshold, pinfo->tbat)) {
			charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
		} else {
			if (pinfo->charge_full && pinfo->soc == 100 && pinfo->ibat > -80 && pinfo->ibat < 80) {
...
static int check_charge_parameters(struct charger_manager *info)
{
	...
	bool bbc_wa = (product_name == PISSARROPRO && info->vbus >= 6500);
...
#define K16U_LOW_FFC_ITEM 650
#define K16U_HIGH_FFC_ITEM 850
static int handle_ffc_charge(struct charger_manager *info)
{
	int ret = 0;
	union power_supply_propval pval = {0,};

	if (!info->recharge && info->entry_soc <= info->ffc_high_soc && is_between(info->ffc_low_tbat, info->ffc_high_tbat, info->tbat) &&
	...
	if (is_between(info->ffc_low_tbat, info->ffc_medium_tbat, info->tbat)) {
		if (product_name == PISSARRO) {
			if (!strncmp(info->device_chem, "ATL", 3))
				info->iterm_ffc = ATL_LOW_FFC_ITEM;
			else if (!strncmp(info->device_chem, "LWN", 3))
				info->iterm_ffc = LWN_LOW_FFC_ITEM;
		} else if (product_name == PISSARROPRO) {
			info->iterm_ffc = K16U_LOW_FFC_ITEM;
		}
	} else if (is_between(info->ffc_medium_tbat, info->ffc_high_tbat, info->tbat)) {
		if (product_name == PISSARRO) {
			if(!strncmp(info->device_chem, "ATL", 3))
			...
			info->iterm_ffc = K16U_HIGH_FFC_ITEM;
		}
	}

	info->iterm_effective = info->ffc_enable ? (info->iterm_ffc - 200) : info->iterm;
	info->iterm_effective = max(info->iterm_effective - info->full_wa_iterm, 100);
	info->fv_effective = info->ffc_enable ? info->fv_ffc : info->fv;
...
{
	static int full_count = 0, recharge_count = 0, full_wa_count = 0;
	int bbc_full = (product_name == PISSARRO) ? MT6360_CHARGE_STATUS_DONE : MP2762_CHARGE_STATUS_DONE;
	int iterm = (info->ffc_enable ? info->iterm_ffc : info->iterm);
	...
	if (info->charge_full) {
		full_count = 0;
		full_wa_count = 0;
		if (info->vbat <= info->fv_effective - (product_name == PISSARRO ? 120 : 200))
			recharge_count++;
		else
			recharge_count = 0;
		...
		recharge_count = 0;
		if ((info->charge_status == bbc_full && (-info->ibat <= info->iterm_effective)) ||
			(info->fg_full && (-info->ibat <= iterm)) ||
			(product_name == PISSARROPRO && info->ffc_enable && info->vbat >= 8930 && (-info->ibat <= iterm))) {
			if (info->soc == 100) {
				full_count++;
				full_wa_count = 0;
				...
				full_wa_count++;
				if (full_wa_count >= 5) {
					info->full_wa_iterm = min(info->full_wa_iterm + 100, 1000);
					full_wa_count = 0;
				}
			}
		} else {
...
static void check_charge_data(struct charger_manager *info)
{
	...
	if (is_between(info->ffc_low_tbat, info->ffc_high_tbat, info->tbat))
		temp = true;
	else
		temp = false;
	if (temp != info->ffc_tbat_normal) {
		info->ffc_tbat_normal = temp;
		info->ffc_tbat_change = true;
	} else
		info->ffc_tbat_change = false;
	...
	if (info->charger_status == CHARGER_PLUGIN || info->ffc_tbat_change)
...
static int mtk_charger_plug_in(struct charger_manager *info)
{
	...
	if (info->pp_enable == info->input_suspend)
		charger_dev_enable_powerpath(info->chg1_dev, !(info->input_suspend));
