...
void setup_selinux(const char *domain, struct cred *cred)
{
	if (transive_to_domain(domain, cred, false)) {
		pr_warn("transive domain failed for '%s', setting per-process permissive as fallback\n", domain);
		// Domain does not exist in this ROM's SELinux policy (e.g. non-GKI kernels
		// without KernelSU su domain). Set per-process permissive so ksud root
		// processes are not blocked by SELinux AVC denials.
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 18, 0)
